...
namespace core {
    ...
    public:
       ...
       for(int xi = 0; xi < dst.cols; xi++) {
        ...
        BirdEyeImg() : ratio(1.0), offset(cv::Point(0.0, 0.0)) { }
        BirdEyeImg(const cv::Mat& _map, const double& _ratio, const cv::Point2d& _offset) :
            map(_map), ratio(_ratio), offset(_offset) { }
        ~BirdEyeImg() { }

        BirdEyeImg<T> clone() const {
        ...
        double ratioDifferenceFrom(const BirdEyeImg<T>& other) const {
