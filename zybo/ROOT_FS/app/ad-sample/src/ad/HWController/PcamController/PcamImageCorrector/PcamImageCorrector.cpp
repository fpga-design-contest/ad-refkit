...
namespace ad {
    ...
    void PcamImageCorrector::execute(const cv::Mat& src,
        ...
        const auto HEIGHT       = map.size().height;
        const auto WIDTH        = map.size().width;
        const auto RD_IM_WIDTH  = WIDTH  * ratio;
        const auto RD_IM_HEIGHT = HEIGHT * ratio;

        for(int yi = 0; yi < HEIGHT; yi++) {
            const auto map_yi = map.ptr<int>(yi);
            for(int xi = 0; xi < WIDTH; xi++) {
                const auto& src_index = map_yi[xi];
                if(0 <= src_index) {
